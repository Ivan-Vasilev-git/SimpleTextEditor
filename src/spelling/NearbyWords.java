...
public class NearbyWords implements SpellingSuggest {
  ...
  public List<String> suggestions(String word, int numSuggestions) {
    ...
    while (!queue.isEmpty()) {
      List<String> distWords = distanceOne(queue.remove(0), true);
      for (String distWord : distWords) {
        if (!visited.contains(distWord)) {
          retList.add(distWord);
          if (retList.size() == numSuggestions || wordsCount >= THRESHOLD) {
            return retList;
          }
          queue.add(distWord);
          visited.add(distWord);
        }
        wordsCount++;
      }
    }
